...
int fast9_corner_score_old(const byte* imp, const int *pointer_dir, int barrier)
{
...
int fast9_corner_score_new(const byte* p, const int pixel[], int bstart)
{    
    int bmin = bstart;
    int bmax = 255;
    int b = (bmax + bmin)/2;
...
{	
  ...
  for(n=0; n < num_corners; n++) {
      if(use_new_score) {
        scores[n] = fast9_corner_score_new(i + corners[n].y*stride + corners[n].x, pixel, b);
      } else {
        scores[n] = fast9_corner_score_old(i + corners[n].y*stride + corners[n].x, pixel, b);
      }
  }
...
template int* fast9_score<true>(const byte* i, int stride, xy* corners, int num_corners, int b);
template int* fast9_score<false>(const byte* i, int stride, xy* corners, int num_corners, int b);
